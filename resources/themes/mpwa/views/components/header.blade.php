             ...
             <ul class="navbar-nav align-items-center">
                 ...
                 <li class="nav-item">
                     @if (Auth::user()->level != 'admin')
                         <a class="nav-link" href="{{ route('user.tickets.index') }}" role="button"
                             style="font-size: 15px;">
                             <span
                                 class="badge rounded-circle bg-danger float-start text-white me-1 d-flex justify-content-center align-items-center"
                                 style="width: 20px; height: 20px;">{{ auth()->user()->tickets()->where('status', 'open')->count() }}</span>
                             {{ __('Tickets') }}
                         </a>
                     @else
                         <a class="nav-link" href="{{ route('admin.tickets.index') }}" role="button"
                             style="font-size: 15px;">
                             <span
                                 class="badge rounded-circle bg-danger float-start text-white me-1 d-flex justify-content-center align-items-center"
                                 style="width: 20px; height: 20px;">{{ \Coderflex\LaravelTicket\Models\Ticket::where('status', 'open')->count() }}</span>
                             {{ __('Tickets') }}
                         </a>
                     @endif
                 </li>
